...
export function ExercisePlayer({ lessonId, lessonTitle, exercises }: Props) {
  ...
  const { completeLesson, loseHeart } = useProgress();
  ...
  if (done) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center gap-6">
        ...
        <h1 className="text-3xl font-black">Awanou ! 🎉</h1>
        <p className="text-muted-foreground max-w-sm">
          Tu as terminé « {lessonTitle} ». AYI est très fier de toi !
        </p>
        <div className="flex gap-3">
          <div className="bg-gold text-gold-foreground rounded-2xl px-5 py-3 font-black shadow-card">
            +{xpGained} XP
          </div>
          <div className="bg-primary text-primary-foreground rounded-2xl px-5 py-3 font-black shadow-card">
            {total - mistakes}/{total}
          </div>
        </div>
        <button
          onClick={() => navigate({ to: "/" })}
          className="btn-3d mt-4 bg-primary text-primary-foreground font-black px-8 py-4 rounded-2xl uppercase tracking-wider"
        >
          Continuer
        </button>
      </div>
      ...
      <div className="flex-1 px-4 py-6 max-w-2xl w-full mx-auto">
        ...
        {ex.type === "choice" && ex.hint && !checked && (
          <button
            ...
            className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-muted-foreground hover:text-foreground"
          >
        ...
        {showHint && ex.type === "choice" && ex.hint && (
          <div className="mt-3">
            <AyiBubble mood="thinking">{ex.hint}</AyiBubble>
          </div>
        )}
      </div>
...
function ExerciseBody({
          ...
          {ex.options.map((o) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                onClick={() => setAnswer(o)}
                className={`btn-3d text-left px-4 py-4 rounded-2xl border-2 font-bold text-base transition-colors ${
                  selected
    ...
    return (
      <>
        <h2 className="text-xl font-bold mb-2">
          Traduis cette {ex.to === "fon" ? "phrase en Fon" : "phrase en français"}
        </h2>
          ...
          {ex.choices.map((c) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                onClick={() => setAnswer(c)}
                className={`btn-3d px-4 py-4 rounded-2xl border-2 font-bold ${
                  selected
                    ? "border-primary bg-primary/10 text-primary"
                    : "border-border bg-card hover:border-primary/50"
                }`}
  ...
  if (ex.type === "order") {
    return <OrderExercise ex={ex} answer={answer as string[] | null} setAnswer={setAnswer} disabled={disabled} />;
  }

  if (ex.type === "match") {
    return <MatchExercise ex={ex} answer={answer as Record<string, string> | null} setAnswer={setAnswer} disabled={disabled} />;
  }
...
function OrderExercise({
  ...
  const remaining = shuffled.filter((w, i) => {
    const usedCount = picked.filter((p) => p === w).length;
    const totalCount = shuffled.filter((x) => x === w).length;
    return picked.indexOf(w) === -1 || usedCount < totalCount && i >= 0;
  });
  // Build available list = shuffled minus picked (respecting duplicates)
      ...
      <div className="min-h-20 border-b-2 border-dashed border-border py-3 mb-6 flex flex-wrap gap-2">
        {picked.map((w, i) => (
          <button
            ...
            className="btn-3d bg-card border-2 border-border px-3 py-2 rounded-xl font-bold"
          >
            {w}
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {available.map((w, i) => (
          <button
            key={i}
            disabled={disabled}
            onClick={() => setAnswer([...picked, w])}
            className="btn-3d bg-card border-2 border-border px-3 py-2 rounded-xl font-bold hover:border-primary/50"
          >
            {w}
          </button>
        ))}
        {remaining.length === 0 && available.length === 0 && null}
      </div>
    </>
  );
}
...
function MatchExercise({
  ...
  setAnswer: (v: Record<string, string>) => void;
  disabled: boolean;
}) {
          ...
          {ex.pairs.map((p) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                onClick={() => pickFr(p.fr)}
                className={`btn-3d w-full px-3 py-3 rounded-xl border-2 font-bold text-sm ${
                  matched
          ...
          {fonShuffled.map((fon) => {
            ...
            return (
              <button
                ...
                onClick={() => pickFon(fon)}
                className={`btn-3d w-full px-3 py-3 rounded-xl border-2 font-bold text-sm ${
                  used ? "opacity-40 bg-muted border-border" : "border-border bg-card hover:border-primary/50"
                }`}
              >
                {fon}
              </button>
    ...
    return (
      <div
        className={`border-t-2 ${
          correct ? "bg-success/15 border-success" : "bg-destructive/10 border-destructive"
        }`}
      >
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className={`font-black text-lg ${correct ? "text-success" : "text-destructive"}`}>
                {correct ? "Awanou ! Bravo !" : "Pas tout à fait..."}
              </div>
              {!correct && (
                <div className="text-sm text-muted-foreground">
                  Bonne réponse : <span className="font-bold">{correctText(ex)}</span>
                </div>
              )}
            </div>
          </div>
          <button
            onClick={onNext}
            className={`btn-3d font-black px-6 py-3 rounded-2xl uppercase tracking-wider ${
              correct
    ...
    <div className="border-t-2 border-border bg-card">
      <div className="max-w-2xl mx-auto px-4 py-4 flex justify-end">
        <button
          onClick={handleCheck}
          disabled={!ready}
          className={`btn-3d font-black px-8 py-3 rounded-2xl uppercase tracking-wider ${
            ready
...
function isReady(ex: Exercise, ans: unknown): boolean {
  ...
  if (ex.type === "match")
    return typeof ans === "object" && Object.keys(ans as object).length === ex.pairs.length;
  return false;
}
...
function isCorrect(ex: Exercise, ans: unknown): boolean {
  ...
  if (ex.type === "order")
    return Array.isArray(ans) && ans.join(" ") === ex.answer.join(" ");
  if (ex.type === "match") {
